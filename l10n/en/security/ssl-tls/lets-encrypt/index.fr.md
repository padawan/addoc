...
Les adresses en _.alwaysdata.net_ sont gérées par le certificat wildcard `*.alwaysdata.net` renvoyé par défaut par les serveurs.
...
La création de ces certificats est **dépendante de la propagation DNS** : il faut que l'adresse pointe sur les serveurs alwaysdata (une vérification HTTP est effectuée). Une fois l'adresse ajoutée dans **Web > Sites**, le système va tenter de générer son certificat toutes les _30 minutes durant 24 heures_. Cela passe ensuite à _une fois par jour_.
